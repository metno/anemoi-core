...
class GraphInterpolator(BaseGraphModule):
    ...
    def get_target_forcing(self, batch: dict[str, torch.Tensor], interp_step: int) -> dict[str, torch.Tensor]:
        ...
        for dataset_name, num_tfi in self.num_tfi.items():
            ...
            if num_tfi >= 1:
                target_forcing[dataset_name][..., :num_tfi] = batch[dataset_name][:, self.imap[interp_step], :, :, self.target_forcing_indices[dataset_name]]
    ...
    def _step(
        ...
        for interp_step in self.interp_times:
            ...
            for dataset_name, dataset_batch in batch.items():
                y[dataset_name] = dataset_batch[:, self.imap[interp_step], :, :, self.data_indices[dataset_name].data.output.full]
